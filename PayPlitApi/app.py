...
# In app.py under the ITEMS SECTION

@app.route("/v1/payments/create", methods=["POST"])
def create_payment():
    """
    🔥 NEW: Record a cash payment between two users.
    Expects: { "itemGroupId", "itemPayer": ["uidA"], "itemSpliter": ["uidB"], "itemTotalAmount": 500 }
    """
    try:
        data = request.get_json(force=True)
        
        # Validation
        if not data.get("itemPayer") or not data.get("itemSpliter"):
            return safe_res("error", "Payer and Receiver required", code=400)
            
        # Add timestamp metadata
        import datetime
        now = datetime.datetime.now()
        data["itemDateUpdate"] = now.strftime("%d-%m-%Y")
        data["itemTimeUpdate"] = now.strftime("%H:%M")
       
        
        # Execute Atomic Payment
        success, msg = repo.create_payment_atomically(data)
        
        if success:
            return safe_res("success", "Payment recorded")
        else:
            return safe_res("error", msg, code=500)
    except Exception:
        return safe_res("error", traceback.format_exc(), code=500)
