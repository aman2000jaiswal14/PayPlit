...
class FirebaseRepository(DatabaseInterface):
    ...
    # Inside firebase_repository.py

    def create_payment_atomically(self, payment_data):
        try:
            # 1. Generate unique ID for the payment record
            item_ref = db.reference("items").push()
            payment_id = item_ref.key
            payment_data["itemId"] = payment_id
            payment_data["itemType"] = "PAYMENT" # 🔥 Industry standard: explicitly mark the type

            group_id = payment_data["itemGroupId"]
            group_ref = db.reference(f"groups/{group_id}")

            def payment_transaction(current_group):
                if current_group is None: return None
                
                # Denormalization: Get names for the receipt
                name_map = current_group.get("memberNames", {})
                p_id = payment_data["itemPayer"][0]
                r_id = payment_data["itemSpliter"][0]
                total_amount = payment_data["itemTotalAmount"]
                payment_data["itemSpliterValue"] = [float(total_amount)]
                payment_data["itemPayerNames"] = [name_map.get(p_id, "Unknown")]
                payment_data["itemSpliterNames"] = [name_map.get(r_id, "Unknown")]
                
                # Automatically set a descriptive name
                payment_data["itemName"] = f"Payment to {payment_data['itemSpliterNames'][0]}"

                # Update Ledger
                from split_logic import update_group_balances, optimal_account_balance
                amount = payment_data["itemTotalAmount"]
                
                # A payment is just A paying B. 
                # update_group_graph logic: A gets +, B gets -
                update_group_balances(current_group, p_id, r_id, amount)
                
                # Re-simplify the graph
                optimal_account_balance(current_group)
                
                # Add to history
                current_group.setdefault("groupItems", []).append(payment_id)
                
                return current_group

            group_ref.transaction(payment_transaction)
            
            # Save the payment record in the items node
            db.reference(f"items/{payment_id}").set(payment_data)
            return True, "payment created"
        except Exception as e:
            return False, str(e)